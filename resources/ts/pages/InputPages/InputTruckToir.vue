...
import Repository from '@/entities/TruckToir/TruckToirRepository';
import TruckRepository from '@truck/TruckRepository';
...
async function onSaveChanges(truck_id, day, change, value) {
   Repository.update({
      value: value,
      day: day,
      truck_id: truck_id,
...
async function getDrillingPositions() {
   ...
   try {
      ...
      formDataModels.value = createToirModels(daysList.value, pageData.excavators, result.data, 'truck_id')
...
onMounted(async () => {
   const { data: excavators } = await TruckRepository.index()
